defmodule Gifmaster.Assets.Url do
  @moduledoc "Builds public original and signed transformation URLs for Gifmaster media."

  @formats [:jpg, :png, :webp]
  @fits [:fit, :fill, :crop]

  def original_url(storage_key) do
    base_url = Application.fetch_env!(:gifmaster, :media_base_url)
    "#{base_url}/original/gifmaster/#{encode_storage_key(storage_key)}"
  end
  ...
  def animated_preview_url(storage_key, version),
    do: variant_url(storage_key, width: 480, height: 480, fit: :fit, quality: 82, format: :webp, version: version)

  def feed_thumbnail_url(storage_key, version),
    do: variant_url(storage_key, width: 320, height: 320, fit: :fill, quality: 78, format: :webp, version: version)
  ...
  def email_image_url(storage_key, version),
    do: variant_url(storage_key, width: 480, height: 480, fit: :fit, quality: 78, format: :webp, version: version)

  defp local_source_url(storage_key), do: "local:///gifmaster/#{encode_storage_key(storage_key)}"
